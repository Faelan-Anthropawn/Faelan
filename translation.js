...
function makeMergeKeyGetter(schem, replacements = {}) {
  // If replacements are passed here, we use them, otherwise we use activeReplacements
  ...
  // Helper to apply manual replacements
...
function isValidBlockId(id) {
  if (!javaToBedrockMap) return true; // Can't validate if not loaded
